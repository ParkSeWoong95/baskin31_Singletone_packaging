...
public class ViewClass {
	...
	private final IUserService iUserService = IUserServiceImpl.getInstance();
	private final ISizeService iSizeService = ISizeServiceImpl.getInstance();
	private final IOrderInformationService iOrderInformationService = IOrderInformationServiceImpl.getInstance();
	private final IOrderDetailsService iOrderDetailsIService = IOrderDetailsServiceImpl.getInstance();
	...
	private String checkId() {
		...
		while (true) {
			id = scanId();
			boolean result = iUserService.checkId(id);
			if (result) {
				break;
			} else {
	...
	private String scanId() {
		...
		while (true) {
			...
			input = sInput();
			if (RegExClass.checkUser_id(input)) {
				break;
			}
	...
	private String scanName() {
		...
		while (true) {
			...
			input = sInput();
			if (RegExClass.checkUser_name(input)) {
				break;
			}
	...
	private String scanPw() {
		...
		while (true) {
			...
			input = sInput();
			if (RegExClass.checkUser_pw(input)) {
				break;
			}
	...
	private void loginView() {
		...
		while (true) {
			...
			if (iAdminService.adminLogin(loginInfo)) {
				adminMainView();
				break;
			} else if (iUserService.userLogin(loginInfo)) {
...
//			adminMainView();
	...
	private void orderInfoView() {
		List<OrderInformationVO> orderList = iOrderInformationService.selectAllOrderInfomation();
		String message = "";
		while (true) {
			System.out.println("⊂==============================================================================⊃");
			if (orderList.size() > 0) {
				for (int i = 0; i < orderList.size(); i++) {
					...
					System.out.print("[ " + (i + 1) + " ] " + "회원 ID : " + orderInformation.getUser_id() + ", 이름 : " + user.getName() + ", 사이즈명 : " + size.getName() + ", 수령 방법 : " + orderInformation.getHowToPick());
	 ...
	 * 
	 * @author 정예진
	 * @param order_seq : 주문을 판별할 수 있는 번호
	 * 
	 */
	private void orderInfoDetails(int order_seq) {
		while (true) {
			...
			SizeVO size = iSizeService.selectSize(order.getSize_seq());
			...
			System.out.print("맛 : ");
			for (OrderDetailsVO orderDetailsVO : details) {
				int kinds = orderDetailsVO.getIcecream_seq();
				IcecreamVO icecream = iIcecreamService.selectIcecream(kinds);
	...
	private void icecreamStockView() {
		String message = "";
		while (true) {
			List<IcecreamVO> icecream = iIcecreamService.selectAllIcecream();
			System.out.println("=============================");
			...
			for (int i = 0; i < icecream.size(); i++) {
				System.out.println("[ " + (i + 2) + " ] " + icecream.get(i).getKinds());
			}
			System.out.println("=============================");
			System.out.println("[ 0 ] 뒤로가기");
			if (!"".equals(message)) {
	...
	private void insertIcecreamMethod() {
		...
		icecream.setSeq(++DBClass.icecream_seq);

		if (iIcecreamService.insertIcecream(icecream)) {
	...
	private void icecreamDetailsView(int icecream_seq) {
		...
		while (true) {
			IcecreamVO icecream = iIcecreamService.selectIcecream(icecream_seq);
	 ...
	 * 
	 * @author 박세웅
	 * @param icecream_seq : icecream을 판별할 수 있는 고유번호
	 */
	private void updateIcecreamInfo(int icecream_seq) {
		...
		while (true) {
			...
			if (iIcecreamService.updateIcecream(icecreamInfo) > 0) {
				message = "변경 성공";
			} else {
				message = "변경 실패";
			}
	...
	private int deleteIcecreamInfo(int icecream_seq) {
		...
		while (true) {
			...
			if ("1".equals(input) || "Y".equals(input) || "y".equals(input)) {
				int result = iIcecreamService.deleteIcecream(icecream_seq);
				if (result > 0) {
				...
				return result;
			} else if ("2".equals(input) || "n".equals(input) || "N".equals(input)) {
	...
	private void sizeManageView() {
		String message = "";
		while (true) {
			List<SizeVO> sizeList = iSizeService.selectAllSize();
			System.out.println("====================");
			System.out.println("[ 1 ] 추가하기");
			System.out.println("====================");
			if (sizeList.size() > 0) {
				for (int i = 0; i < sizeList.size(); i++) {
					System.out.println("[ " + (i + 2) + " ] " + sizeList.get(i));
				}
			} else {
				System.out.println();
	 ...
	 * 
	 * @author 이학재
	 * @param size_seq : size를 판별할 수 있는 고유번호
	 */
	private void sizeDetailsView(int size_seq) {
		String message = "";
		while (true) {
			SizeVO size = iSizeService.selectSize(size_seq);
			System.out.println("========================");
	...
	private void updateSizeInfo(int size_seq) {
		...
		while (true) {
			...
			if (iSizeService.updateSize(sizeInfo) > 0) {
				message = "변경에 성공하였습니다.";
			} else {
				message = "변경에 실패하였습니다.";
			}
	...
	private int deleteSizeInfo(int size_seq) {
		...
		while (true) {
			...
			String input = sInput();
			if ("1".equals(input) || "Y".equals(input) || "y".equals(input)) {
				int result = iSizeService.deleteSize(size_seq);
				if (result > 0) {
	...
	private void userListView() {
		List<UserVO> userList = iUserService.selectAllUser();
	 ...
	 * 
	 * @author 정예진
	 * @param user_id : user를 판별하기 위한 id
	 */
	private void userDetailsView(String user_id) {
		UserVO userInfo = iUserService.selectUser(user_id);
		String message = "";
		while (true) {
		...
		while (true) {
			...
			if (notifyList.size() > 0) {
				for (int i = 0; i < notifyList.size(); i++) {
					System.out.println("[ " + (i + 2) + " ] " + notifyList.get(i));
				}
			} else {
	 ...
	 * 
	 * @author 박세웅
	 * @param notify_seq : 공지를 판별하기 위한 고유번호
	 */
	private void notifyDetailsView(int notify_seq) {
		String message = "";
		while (true) {
			NotifyVO notify = iNotifyService.selectNotify(notify_seq);
			System.out.println("────────────────────────────────");
	...
	private void updateNotify(int notify_seq) {
		...
		while (true) {
			...
			if (iNotifyService.updateNotify(notifyObj) > 0) {
				message = "변경에 성공하였습니다.";
			} else {
				message = "변경실패";
			}
	...
	private int deleteNotify(int notify_seq) {
		...
		while (true) {
			...
			String input = sInput();
			if ("1".equals(input) || "Y".equals(input) || "y".equals(input)) {
				int result = iNotifyService.deleteNotify(notify_seq);
				if (result > 0) {
					System.out.println("삭제 완료");
					return 1;
	...
	private void userNotifyView() {
		...
		while (true) {
			...
			if (notifyList.size() > 0) {
				for (int i = 0; i < notifyList.size(); i++) {
					System.out.println("[ " + (i + 1) + " ] " + notifyList.get(i));
				}
			} else {
				System.out.println("공지사항이 없습니다.");
			}
	...
	private void pointCharge() {
		...
		int result = iUserService.addPoint(userObj);
		if (result <= 0) {
		...
		while (true) {
			...
			if (input == 0) {
				...
				if (user.getPoint() < sizeList.get(input - 1).getPrice()) {
					...
					user.setPoint(user.getPoint() - sizeList.get(input - 1).getPrice());
	 ...
	 * 
	 * @author 이학재
	 * @param size_seq : 사이즈 설정을 위한 사이즈의 고유번호
	 */
	private void orderDetails(int size_seq) {
		...
		int order_seq = ++DBClass.orderInformation_seq;
		while (true) {
			...
			if (orderInformation.getSpoonCount() == -1) {
				...
				System.out.println("1. 숟가락 개수 : " + orderInformation.getSpoonCount() + "개");
				System.out.println("→ 2. 수령 방법");
				for (int i = 0; i < size.getFlavorKinds(); i++) {
					System.out.println(i + 1 + "번째 맛 선택");
				}
			} else {
				System.out.println("1. 숟가락 개수 : " + orderInformation.getSpoonCount());
				System.out.println("2. 수령 방법 : " + orderInformation.getHowToPick());
				for (int i = 0; i < size.getFlavorKinds(); i++) {
					if (i < orderDetailsList.size()) {
						System.out.println(i + 1 + "번째 맛 : " + iIcecreamService.selectIcecream(orderDetailsList.get(i).getIcecream_seq()).getKinds());
			...
			if (size.getFlavorKinds() != orderDetailsList.size()) {
				...
				orderDetails.setSeq(++DBClass.orderDetails_seq);
				orderDetailsList.add(orderDetails);
				continue;
			}
			orderInformation.setSeq(order_seq);

			if (!iOrderInformationService.insertOrderInformation(orderInformation)) {
				return;
			}
			if (!iOrderDetailsIService.insertOrderDetails(orderDetailsList)) {
				return;
			}
			break;
		}
	...
	private OrderDetailsVO selectFlavor(int size_seq) {
		...
		while (true) {
			...
			System.out.println("­---------------------------");
			for (int i = 0; i < icecream.size(); i++) {
				System.out.println("[" + (i + 1) + "]" + icecream.get(i).getKinds());
			}
			System.out.println("­---------------------------");
			int input = iInput();
			if (0 < input && input < icecream.size() + 1) {
				if (size.getGram() / size.getFlavorKinds() > icecream.get(input - 1).getStock()) {
					message = "해당 아이스크림의 재고가 없어 선택할 수 없습니다.";
					continue;
				}
				Map<String, Object> order = new HashMap<>();
				order.put("icecream_seq", icecream.get(input - 1).getSeq());
				order.put("icecream_stock", icecream.get(input - 1).getStock() - size.getGram() / size.getFlavorKinds());
				iIcecreamService.updateIcecream(order);
	...
	private void orderHistoryView() {
		while (true) {
			List<OrderInformationVO> orderList = iOrderInformationService.selectAllOrderInformation(user.getId());
			if (orderList.size() == 0) {
	 ...
	 * 
	 * @author 정예진
	 * @param order_seq : 주문을 판별하기 위한 고유번호
	 * 
	 */
	 ...
	 * 
	 * @author 정예진
	 * @param orderInfo_seq : 주문을 판별하기 위한 고유번호
	 * @return 환불 성공시 1, 실패 시 0
	 */
	private int refund(int orderInfo_seq) {
		// 환불
		int orderInfo = iOrderInformationService.refundOrder(orderInfo_seq);

		// 유저에게 돈 돌려주기
		OrderInformationVO order = iOrderInformationService.selectOrderInfomation(orderInfo_seq);
		SizeVO size = iSizeService.selectSize(order.getSize_seq());
		...
		if (orderInfo > 0) {
			...
			iUserService.updateUser(params);
	...
	private void deleteUser() {
		...
		while (true) {
			...
			String input = sInput();
			if ("1".equals(input) || "Y".equals(input) || "y".equals(input)) {
				int result = iUserService.deleteUser(user.getId());
				if (result > 0) {
